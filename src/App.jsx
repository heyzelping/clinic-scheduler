...
.sync-bar { display:flex;align-items:center;gap:8px;padding:5px 12px;border-radius:6px;font-size:11px; }
.sync-bar.connected { background:rgba(56,217,169,.1);color:var(--accent2);border:1px solid rgba(56,217,169,.2); }
.sync-bar.demo { background:rgba(255,169,77,.1);color:var(--warn);border:1px solid rgba(255,169,77,.2); }
.sync-bar.saving,.sync-bar.loading { background:rgba(91,141,238,.1);color:var(--accent);border:1px solid rgba(91,141,238,.2); }
.sync-dot { width:6px;height:6px;border-radius:50%;background:currentColor;flex-shrink:0; }
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
.sync-dot.active { animation:pulse 1.2s infinite; }
`;
...
const [syncStatus, setSyncStatus] = useState(isConfigured() ? "loading" : "demo");
...
Promise.all([apiGet("getStaff"), apiGet("getLeave")])
.then(([s, l]) => {
if (s && !s.error && s.length) setStaff(s.map(x => ({ ...x, offDays: Array.isArray(x.offDays) ? x.offDays : (x.offDays||"").split(",").map(d=>d.trim()).filter(Boolean) })));
if (l && !l.error) setLeaveLog(l);
setSyncStatus("connected");
}).catch(() => setSyncStatus("demo"));
}, []);
...
syncStatus={syncStatus} saving={saving}
/>
...
// ─── TOPBAR ───────────────────────────────────────────────────────────────────
function Topbar({ tab, viewYear, viewMonth, setViewYear, setViewMonth, onAddLeave, onAddStaff, syncStatus, saving }) {
...
<div className={`sync-dot${syncStatus === "loading" || saving ? " active" : ""}`}/>
{saving ? "Saving to Sheets…" : syncStatus === "connected" ? "Connected to Sheets" : syncStatus === "loading" ? "Connecting…" : "Demo mode"}
</div>
...
const updated = leaveLog.find(x => x.id === id);
const entry = { ...updated, status: "Approved", approvedBy: "Manager" };
setLeaveLog(l => l.map(x => x.id === id ? entry : x));
if (isConfigured()) { setSaving(true); await apiPost({ action: "updateLeave", data: entry }); setSaving(false); }
}
async function reject(id) {
const updated = leaveLog.find(x => x.id === id);
const entry = { ...updated, status: "Rejected" };
setLeaveLog(l => l.map(x => x.id === id ? entry : x));
if (isConfigured()) { setSaving(true); await apiPost({ action: "updateLeave", data: entry }); setSaving(false); }
}
async function remove(id) {
setLeaveLog(l => l.filter(x => x.id !== id));
if (isConfigured()) { setSaving(true); await apiPost({ action: "deleteLeave", id }); setSaving(false); }
}
...
if (isConfigured()) await apiPost({ action: "updateStaff", data: updated });
}
return (
<div>
...
<div className="staff-card-id">{s.id} · {s.branch === "Float" ? "Float" : `${s.branch === "A" ? "Orchard" : s.branch === "B" ? "Katong" : "Float"}`}</div>
</div>
...
if (isConfigured()) await apiPost({ action: "updateLeave", data: { ...form, id: editLeave.id } });
} else {
const id = "LV" + String(leaveLog.length + 1).padStart(3, "0");
const entry = { ...form, id };
setLeaveLog(l => [...l, entry]);
if (isConfigured()) await apiPost({ action: "saveLeave", data: entry });
}
onClose();
}
return (
...
const member = { ...form, id };
setStaff(s => [...s, member]);
if (isConfigured()) await apiPost({ action: "saveStaff", data: member });
onClose();
}
return (
<div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
